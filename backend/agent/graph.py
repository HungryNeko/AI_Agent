...
def conversation_begin(state: ChatState) -> dict[str, Any]:
    ...
    user_parts = [
        part
        for part in [time_text, context_text, tool_text, f'userMessage: "{state["message"]}"']
        if part
    ]
...
def assistant_step(state: ChatState) -> dict[str, Any]:
    ...
    if state.get("tool_rounds", 0) >= state.get("max_tool_rounds", 20):
        messages.append(
            {
                "role": "user",
                "content": (
...
def tool_call(state: ChatState) -> dict[str, Any]:
    ...
    for request in tool_requests:
        key = tool_call_key(request)
        previous_count = tool_call_counts.get(key, 0)
        if previous_count >= 2:
            result = format_repeated_tool_call_error(request, previous_count)
        else:
            tool_call_counts[key] = previous_count + 1
